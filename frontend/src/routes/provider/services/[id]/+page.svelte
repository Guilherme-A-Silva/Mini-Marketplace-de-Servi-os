<script>
  ...
  onMount(async () => {
    await Promise.all([loadServiceTypes(), loadService()]);
  });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await api.put(`/services/${$page.params.id}`, {
        name,
        description,
      ...
      error = err.response?.data?.error || 'Erro ao atualizar serviço';
...
<svelte:head>
  <title>Editar Serviço - Marketplace</title>
</svelte:head>
...
{:else if service}
  <div class="max-w-3xl mx-auto px-2 md:px-4">
    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-6 md:mb-8">Editar Serviço</h1>
